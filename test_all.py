...
#Hàm xử lý phép tính tiếp theo, sau khi ấn dấu bằng
def next_cal():
...
def press_num(number):
    ...
    if operator == '' and first_number == '' and second_number == '' and display.get() != '':
        next_cal()
...
def press_point():
    global operator, first_number, second_number
    if display.get() != '' and first_number == '' and second_number == '' and operator == '':
        display.config(state="normal")
        display.delete(0, END)
        first_number = ''
        second_number = ''
        operator = ''
        display.config(state="disabled")
    elif display.get() != '' and '.' not in display.get():
        display.config(state="normal")
